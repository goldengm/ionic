...
export class SharedDataProvider {
  ...
  public auctionProducts: any;
  public categories = new Array();
  public subCategories = new Array();
  public buyerNavigation = new Array();
  ...
  constructor(
    ...
    data.type = 'auctions';
    this.httpClient.post(this.config.url + 'getallproducts', data).subscribe((data: any) => {
      this.auctionProducts = data.product_data
    });
    ...
    this.httpClient.post(config.url + 'buyerNavigation', { language_id: config.langId }).subscribe((data: any) => {
      for (let value of data.data) {
        if (value.parent_id == 0) this.buyerNavigation.push(value);
        else this.subCategories.push(value);
      }
    });
